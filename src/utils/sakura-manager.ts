...
type SakuraUpdateFn = (value: number, secondary?: number) => number;
...
function getRandom(
	option: string,
	config: SakuraConfig,
): number | SakuraUpdateFn {
	let result: number | SakuraUpdateFn = 0;
	let random = 0;

	switch (option) {
		case "x":
			result = Math.random() * window.innerWidth;
			break;
		case "y":
			result = Math.random() * window.innerHeight;
			break;
		case "s":
			result =
		...
		case "fnx":
			...
			result = (value: number) => value + random;
			break;
		case "fny":
			random =
			...
			break;
		case "fnr":
			result = (rotation: number) => rotation + config.speed.rotation;
			break;
		case "fna":
			result = (alpha: number) => alpha - config.speed.fadeSpeed * 0.01;
			break;
	}
	return result;
}

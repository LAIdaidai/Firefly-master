...
if (typeof window !== "undefined") {
	...
	if (swupInstance?.hooks) {
		swupInstance.hooks.on("content:replace", handleContentReplace);
	} else {
		document.addEventListener("swup:enable", () => {
			const loadedSwup = window.swup;
			if (loadedSwup?.hooks) {
				loadedSwup.hooks.on("content:replace", handleContentReplace);
			}
		});
	}
